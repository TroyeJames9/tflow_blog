...
该部分内容主要参考 [官方文档](https://github.com/rime/home/wiki/UserGuide#%E5%90%8C%E6%AD%A5%E7%94%A8%E6%88%B6%E8%B3%87%E6%96%99) 与 [多平台开源输入法 RIME 的使用](https://zhuanlan.zhihu.com/p/603692461)

借助移动存储设备，或在线存储服务如 Dropbox，开源同步软件syncthing 等 在多台电脑及不同系统之间**同步用户词典和用户设定**。

我将以syncthing为例子介绍rime的同步流程。假设同步设备数量为4。

在阅读以下流程前，请先确保掌握[syncthing](杂记/软件/syncthing.md) 的安装与使用。

### 硬件角色分配与 RIME 基础配置

1. **指定枢纽机**：从4台中选一台 **最常开机、且性能稳定** 的设备作为“枢纽”（例如你的台式机或NAS）。另外3台称为“边缘机”（笔记本、手机、办公电脑）。
2. **修改 `installation.yaml`**（关键）：在4台设备的RIME用户目录下找到此文件，将 `installation_id` 修改为 **4个绝对不同的清晰名称**。示例：`Hub-PC`、`Edge-Laptop`、`Edge-Mac`、`Edge-Phone`。
3. **统一同步目录**：在4台设备的 `installation.yaml` 将 `sync_dir` 的路径**统一指向**你打算用Syncthing监控的文件夹（例如 `D:/RimeSync` 或 `~/RimeSync`）。**路径可以不同**（Win和Mac路径不同没关系），但逻辑上必须是Syncthing共享的同一个文件夹。 一般路径保持默认即可，除非默认路径没有权限则可以指定其他公共文件夹。

### Syncthing 的“星形”连接（只连枢纽）

**切记：边缘机之间绝对不要互相添加！只允许边缘机连接枢纽！**

1. **在枢纽机上**：
    - 打开Syncthing界面，点击“添加设备”。
    - 依次粘贴 **3台边缘机** 的55位设备ID，分别命名，点击保存。
    - **此时先不要动边缘机**，枢纽机界面会显示“正在等待连接”。
2. **在三台边缘机上**：
    - 分别打开Syncthing界面，此时屏幕**顶部或右下角会弹出黄色横幅**：“枢纽机 想要添加您”。
    - **点击横幅上的“添加设备”**，确认连接。
    - （如果没弹窗，手动点“添加设备”，反过来粘贴枢纽机的ID也行，但弹窗确认是最稳的）。
3. **检查连接状态**：在枢纽机上，看到3台边缘机全部显示 **“已连接”（绿色）**，星形网络搭建完成。

### 共享 RIME 同步文件夹

1. **在枢纽机上**：添加你刚刚设定的 `RimeSync` 文件夹，点击右侧的“编辑” -> “共享”选项卡。
2. 在弹出的设备列表中，**勾选全部3台边缘机**，点击保存。
3. **在每台边缘机上**：会收到“是否接受文件夹”的提示，点击“添加”。
    - **重点**：在弹出的路径选择框中，**必须**选择你在RIME里设定的那个 `sync_dir` 本地路径（例如边缘机的 `D:/RimeSync`）。
4. 等待Syncthing扫描完成，界面显示“已同步（Up to Date）”。

### 首次初始化（建立“基准备份”）

这是决定成败的“冷启动”顺序，**必须在枢纽机上先做**：

1. **只在枢纽机上**，打开RIME输入法，点击 **“同步”** 按钮。
    - 此时，枢纽机的 `RimeSync` 文件夹下会生成一个子文件夹，名为 `Hub-PC/`（里面是枢纽机的词库快照）。
2. **等待** Syncthing 把 `Hub-PC/` 这个文件夹完整传输给另外3台边缘机（观察Syncthing界面进度条变为0）。
3. **在3台边缘机上**，依次打开RIME，点击 **“同步”**。
    - 此时边缘机会读取枢纽机传来的 `Hub-PC/` 文件，并生成自己的 `Edge-XXX/` 子文件夹。
4. **再次等待** Syncthing 把这3个边缘机的新文件夹传回枢纽机。至此，枢纽机上拥有**全部4台设备的独立备份子文件夹**，初始基准建立完成。

### 每日/每次使用的标准动作（星形拓扑专用流程）

在星形拓扑下，边缘机之间不直接通信，所有数据必须经过枢纽合并。因此你的操作流程是：

**场景A：当你用任意一台“边缘机”时（例如笔记本）**

- 开机后：**先看Syncthing**，等待它显示“已同步”（确保从枢纽机拉取了其他设备的最新增量）。
- 然后点击RIME的“同步”（此时RIME会读取本地 `RimeSync/` 下所有4台设备的子文件夹，进行逻辑合并）。
- 使用完毕关机前：**再次点击RIME的“同步”**（生成最新的增量文件）。
- 最后：**不关Syncthing**，等它把刚更新的 `Edge-Laptop/` 文件夹推送回枢纽机。

**场景B：当枢纽机开机时（枢纽机必须承担“合纵连横”的责任）**

- 枢纽机开机后，先等Syncthing把3台边缘机在外产生的所有增量文件拉取到本地。
- **在枢纽机上点击一次RIME“同步”**（起到“总合并”的作用，将3台机器的词频汇总）。
- 此时枢纽机会生成一份“集大成”的增量文件，Syncthing会自动把它分发给3台边缘机。

###  针对这个拓扑的“铁律”

1. **枢纽机要当“路由器”**：如果枢纽机不开机，3台边缘机之间**无法交换词库**（因为它们没互相连）。此时在边缘机上打字产生的词频，会暂时存在本地 `Edge-XXX/` 文件夹里，等枢纽机上线后再合并，**不会丢失**。
2. **<span style="color: #ff7575">绝对禁止“同时点同步”</span>** ：不要在边缘机A点完同步后，不等Syncthing传完，立刻去边缘机B点同步。会给枢纽带来“时间戳错乱”的风险。**标准间隔：点完同步后，至少等Syncthing传输完毕再操作下一台**。
3. **只部署，不同步**：如果你修改了方案（改词库、改字体），只需要点击 **“重新部署”**，不要没事就点“同步”。同步只用于交换**用户词频和用户词典**。

![|842](Afile/RIME开源输入法-20260723201555943.webp)
...
- 同步方案来源： [多平台开源输入法 RIME 的使用](https://zhuanlan.zhihu.com/p/603692461)
