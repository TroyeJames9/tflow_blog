...
- **设备本身具备访问外网（已开启代理/VPN）的能力**（教程参照[VPN](杂记/关于代理/VPN.md)) ，从而访问以下所涉及的所有外网资源。
...
- iOS安装包：app store搜索[clash mi](https://apps.apple.com/us/app/clash-mi/id6744321968)下载（需要苹果外区ID，[点击查看教程](杂记/关于代理/美区%20Apple%20ID%20注册教程.md)）
...
> [!EXAMPLE] 需要代购？
> [联系tflow](杂记/如何联系tflow？.md)购买5元/月的性价比订阅，提供一切指导。
...
最后如图所示打开代理以及进行必要配置，就可以无感访问外网了。**（下面还有测速教程哦）**
...
浏览器访问 www.google.com ，代理配置成功则不会报错
...
添加成功后，会跳转到 `我的配置`的页面，如下所示，然后返回APP首页, 滑动打开连接按钮 ▶️ 即可开始您的科学上网之旅了 **（下面还有测速教程哦）**
...
点击软件中间的`代理`，然后右上角有一个闪电图标，点击他就是对所有节点测速，每个节点下方的数字就是这个节点的延迟，如果是红色就代表该节点不可用，以后就可以自己排查了

点击下图红框的策略组，然后在弹出的proxy列表中随意选择延迟低的proxy即可
