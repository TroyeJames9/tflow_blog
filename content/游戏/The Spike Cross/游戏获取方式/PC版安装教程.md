---
title: the spike cross PC版安装教程
aliases:
  - TheSpike/download_pc
---
本文章永久分享链接： https://tflow.top/TheSpike/download_pc

这款游戏需要支持访问外网（翻墙）的设备才能游玩，因为：
- 由韩国团队开发，游戏只在steam上架。
- 游戏登陆需要google账号。所以普通的游戏加速器无法加速google登陆，比如UU加速器
- 所以需要在PC端配置代理/梯子才能登陆

下面将逐步讲解如何令设备满足以上前置条件并获取游戏。**请严格按照步骤来操作**

**如果看不懂，懒得看懂，不会弄的**，可以[联系tflow](杂记/如何联系tflow？.md) 进行付费全指导，附赠一切相关资源。

# 配置并开启代理（proxy）

中国大陆设有防火墙，访问、登陆google时你需要配置代理以访问境外资源。具体原理不做解释，如有需求自行搜索。

配置代理有两种方式：使用傻瓜式VPN（付费或免费）、使用开源网络代理工具（比如mihomo）。**<span style="color: #ff7575">强烈推荐后者，傻瓜式有钓鱼风险。</span>**

如果你从未学习并配置过代理，点击阅读[本教程](杂记/关于代理/VPN.md)学习并开启代理，**后续步骤要确保代理一直处于开启状态**。

> [!warning] 警告
> - 代理相关话题禁止在q群内讨论，如对本部分教程有疑问或补充请[联系tflow](如何联系tflow？.md)
> - **科学上网有一定风险，请不要在墙外发表不当言论或从事任何违法活动！！！**

> [!INFO] 更多教程
> 如果觉得免费甚至付费的VPN不够安全、实惠或稳定，请学习[开源代理工具配置](开源代理工具配置.md)

# 注册谷歌账号

> [!tip] 需要代注册账号？
> 如嫌麻烦、没电脑或无法注册，强烈建议[联系tflow](app://obsidian.md/%E6%9D%82%E8%AE%B0/%E5%A6%82%E4%BD%95%E8%81%94%E7%B3%BBtflow%EF%BC%9F.md)代注册谷歌账号（10元，如接码则20元，包售后退款），太多人在其他途径买到黑号被风控而丢失了，

本部分参考了[注册教程1](https://shuziyimin.net/account/google)和[接码教程2](https://shuziyimin.net/skill/sms-activate)，特别鸣谢。

打开浏览器，为避免国内+86手机号可能遇到的无法验证的问题，建议在**电脑端**使用**Chrome浏览器**，打开VPN代理，并进行以下设置：

- **使用无痕模式**：打开 Chrome 浏览器的`无痕模式`，避免旧有 cookie 的干扰。
- **切换环境语言**：将设备或浏览器的语言设置为 `English`，地区设置为美国或其他国家。
- 地址栏输入[https://accounts.google.com/](https://accounts.google.com/) 进入账号注册页
- 开始创建账户，点击Create account（创建账户）--For my personal use（供我个人使用）
- 填写个人信息，依次填写后点击Next：
- **出生月、日、年**（随便选个已成年日期）
- 验证手机号时，选择+86中国地区

> [!TIP] 提示
> - **注册成功后请注意**：在最初的一周内，尽量使用同一个网络节点登录和使用 Google 服务，避免因 IP 大幅跳动而触发二次手机验证。
> - 务必要牢记账号和密码，强烈建议加上**两步验证（谷歌验证器）、备用码、辅助邮箱、辅助电话号**等安全措施，**<span style="color: #ff7575">同时要确保[谷歌账号页面的个人信息](https://myaccount.google.com/personal-info)里的电话号码是自己的！</span>** 否则一旦丢失、被封号或风控，很难找回账号
> - **如果手上可用的电话号注册均失败**，可选择：
> 	- 认真阅读[谷歌账号注册](杂记/关于代理/谷歌账号注册.md)并尝试
> 	- [联系tflow](杂记/如何联系tflow？.md)代注册谷歌账号

# 注册并安装steam

访问 https://store.steampowered.com/ 来注册并安装steam，建议使用gmail邮箱（即上一步注册的google）

应该比较简单，不赘述了

# 在steam中搜索游戏并安装

如图所示，搜索框搜索 `the spike cross`

![|400](Afile/the%20spike%20cross%20PC版安装教程-20250920092920384.webp)

# 开始游戏吧！

![|400](Afile/the%20spike%20cross%20PC版安装教程-20250920093132331.webp)