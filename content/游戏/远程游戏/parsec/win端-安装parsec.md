...
> [!warning] 学习本文的前提条件
> 掌握[开源代理工具配置](线上移民/开源代理工具配置.md)的内容，**<span style="color: #ff7575">以下教程与parsec使用期间需要你的相关设备保持代理为开启状态</span>**
...
其中`Virtual Display Driver`用于在主机上创建虚拟显示器，对于无显示器的系统很有用。这也可以稍后在 Parsec 设置中安装 虚拟显示驱动程序 ：此驱动程序用于在主机上创建虚拟显示器，适用于无头系统。您也可以稍后在 Parsec 设置中安装此驱动程序。
...
首先要**确保你的主机允许客户端连接**：单击左侧“设置”齿轮按钮->`Host`->`Hosting Enabled`设置为`enabled`。



# 作为客户端连接朋友主机

可以选择两种方式： `直接添加好友`或`通过邀请链接`
