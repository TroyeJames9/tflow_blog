...
- NAT1：完全锥形-Full Cone
- NAT2：限制锥形-Restricted Cone 与 端口限制锥形-Port Restricted Cone
- NAT3：对称-Symmetric
...
| 端口限制锥形NAT2 | 端点无关映射+地址和端口相关过滤    |
| 对称NAT3     | 地址和端口相关映射+地址和端口相关过滤 |
NAT映射行为有3种，NAT过滤行为也有3种，完整组合有9种，上面只列出了4种，剩余5种哪里去了？

首先需要理解一点，映射行为比过滤行为更严格是没有意义的。因为更严格的映射应为需要占用更多的资源，而宽松的过滤行为并没有提升安全性。所以，我们可以排除其他的5种组合。
