...
不过强调一下，**<span style="color: #ff7575">地址相关映射+端点无关过滤 行为模式的STUN打洞成功率会比端口限制锥形更高</span>**
，剩余的4个STUN打洞难度都要更高。

有了以上知识储备，就不难得出如下STUN打洞难度的说明。

# P2P可行性参考表
分以下三个等级↓

- `优`：理想P2P条件。有一方或双方无需NAT（即NAT0），直连。
...
- `差`：对打洞时序要求严格，成功率中等，可能需要依赖TURN中继
	- 任意一方不为NAT1或者NAT2或者NAT3。
	- 任意一方为NAT4，且另一方没有公网或不支持IPv6。
